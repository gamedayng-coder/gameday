...
import type { LinkedInCredential, LinkedInPost } from "@/lib/linkedin-db";
import type { Poster } from "@/lib/poster-db";

interface Props {
  credential: LinkedInCredential | null;
  posts: LinkedInPost[];
...
export default function LinkedInAdminClient({ credential: initial, posts: initialPosts, approvedPosters }: Props) {
  ...
  async function handleDisconnect() {
    ...
    const res = await fetch("/api/linkedin/auth", { method: "DELETE" });
    if (res.ok) {
      setCredential(null);
  ...
  async function handleProcessQueue() {
    setMessage(null);
    const res = await fetch("/api/linkedin/process-queue", { method: "POST" });
    if (res.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-900">LinkedIn</h1>
        {credential && (
          <button
        ...
        {credential ? (
          ...
          <div className="flex items-center justify-between">
            ...
            <a
              ...
              className="rounded-full bg-[#0077B5] text-white text-sm px-4 py-2 font-medium hover:opacity-90 transition-opacity"
            >
              Connect LinkedIn
            </a>
          </div>
          ...
          <form onSubmit={handleCompose} className="space-y-3">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="rounded-full bg-[#0077B5] text-white text-sm px-5 py-2 font-medium hover:opacity-90 disabled:opacity-40 transition-opacity"
              >
