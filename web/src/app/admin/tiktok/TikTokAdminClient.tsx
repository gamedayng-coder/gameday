...
import type { TikTokCredential, TikTokPost } from "@/lib/tiktok-db";
import type { Poster } from "@/lib/poster-db";

interface Props {
  credential: TikTokCredential | null;
  posts: TikTokPost[];
...
export default function TikTokAdminClient({ credential: initial, posts: initialPosts, approvedPosters }: Props) {
  const [credential, setCredential] = useState(initial);
  const [posts, setPosts] = useState<TikTokPost[]>(initialPosts);
  ...
  async function handleDisconnect() {
    ...
    const res = await fetch("/api/tiktok/auth", { method: "DELETE" });
    if (res.ok) {
      setCredential(null);
  ...
  async function handleCancel(id: number) {
    const res = await fetch(`/api/tiktok/posts/${id}`, { method: "DELETE" });
    if (res.ok) {
  ...
  async function handleProcessQueue() {
    setMessage(null);
    const res = await fetch("/api/tiktok/process-queue", { method: "POST" });
    if (res.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-900">TikTok</h1>
        {credential && (
          <button
        ...
        {credential ? (
          ...
          <div className="flex items-center justify-between">
            ...
            <a
              ...
              className="rounded-full text-white text-sm px-4 py-2 font-medium hover:opacity-90 transition-opacity"
        ...
        <div className="rounded-xl border border-zinc-200 p-5 space-y-4">
          ...
          <form onSubmit={handleCompose} className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-zinc-500 mb-1">
                  Attach poster <span className="text-red-400">(required)</span>
                </label>
                <select
                  value={posterId}
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={submitting || !content.trim() || content.length > MAX_LENGTH || !posterId}
                className="rounded-full text-white text-sm px-5 py-2 font-medium hover:opacity-90 disabled:opacity-40 transition-opacity"
                style={{ backgroundColor: TIKTOK_COLOR }}
              >
