...
import type { TelegramCredential, TelegramPost } from "@/lib/telegram-db";
...
export default function TelegramAdminClient({ credential: initial, posts: initialPosts, approvedPosters }: Props) {
  const [credential, setCredential] = useState(initial);
  const [posts, setPosts] = useState<TelegramPost[]>(initialPosts);
  ...
  async function handleConnect(e: React.FormEvent) {
    ...
    if (res.ok) {
      const data = await res.json() as { botUsername: string; chatId: string };
      // Re-fetch credential by reloading page data via status endpoint
      const statusRes = await fetch("/api/telegram/status");
      if (statusRes.ok) {
        const status = await statusRes.json() as { connected: boolean; botUsername: string | null; chatId: string };
        if (status.connected) {
          setCredential({
            id: 0,
            bot_token: "",
      ...
      setBotToken("");
      setChatId("");
      setMessage(`Connected @${data.botUsername} to channel ${data.chatId}.`);
    } else {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="rounded-xl border border-zinc-200 p-5 space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 uppercase tracking-wide">Bot Connection</h2>
        {credential ? (
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-zinc-900">
                {credential.bot_username ? `@${credential.bot_username}` : "Bot connected"}
              </p>
              <p className="text-xs text-zinc-400 mt-0.5">
                Channel: <code className="bg-zinc-100 px-1 rounded">{credential.chat_id}</code>
              </p>
            </div>
            <button
          ...
          <form onSubmit={handleConnect} className="space-y-3">
            <p className="text-sm text-zinc-500">
              Create a bot via{" "}
              <span className="font-medium text-zinc-700">@BotFather</span>, add it to your channel as an admin, then enter the credentials below.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-300"
                />
              </div>
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Channel ID or username</label>
                <input
                  type="text"
                  value={chatId}
            ...
            <div className="flex justify-end">
              <button
                ...
                className="rounded-full bg-[#2AABEE] text-white text-sm px-4 py-2 font-medium hover:opacity-90 disabled:opacity-40 transition-opacity"
              >
          ...
          <form onSubmit={handleCompose} className="space-y-3">
            <div>
              <textarea
                ...
                placeholder="Write a message or caption…"
            ...
            <div className="flex justify-end">
              <button
                ...
                className="rounded-full bg-[#2AABEE] text-white text-sm px-5 py-2 font-medium hover:opacity-90 disabled:opacity-40 transition-opacity"
              >
      ...
      {posts.length > 0 && (
        <div>
          ...
          <p className="mt-2 text-xs text-zinc-400">
            Call{" "}
            <code className="bg-zinc-100 px-1 rounded">POST /api/telegram/process-queue</code>{" "}
      ...
      {!credential && posts.length === 0 && (
        <div className="text-center py-12 text-zinc-400 text-sm">
          Connect a Telegram bot to start posting.
        </div>
      )}
    </div>
  );
