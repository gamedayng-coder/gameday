  ...
  for (const code of competitionCodes) {
    try {
      ...
      for (const m of upcomingMatches) {
        const homeTeam = await upsertTeam(String(m.homeTeam.id), m.homeTeam.name, m.homeTeam.shortName ?? null, m.homeTeam.tla ?? null, m.homeTeam.crest ?? null);
        const awayTeam = await upsertTeam(String(m.awayTeam.id), m.awayTeam.name, m.awayTeam.shortName ?? null, m.awayTeam.tla ?? null, m.awayTeam.crest ?? null);
        const matchSeason = m.season ? `${m.season.startDate.slice(0, 4)}/${m.season.endDate.slice(0, 4)}` : season;
        await upsertFixture(String(m.id), comp.id, homeTeam.id, awayTeam.id, m.utcDate, m.venue ?? null, m.status, m.score?.fullTime?.home ?? null, m.score?.fullTime?.away ?? null, m.matchday ?? null, matchSeason);
      }
      ...
      if (season) {
        ...
        if (totalTable) {
          for (const row of totalTable.table) {
            const team = await upsertTeam(String(row.team.id), row.team.name, row.team.shortName ?? null, row.team.tla ?? null, row.team.crest ?? null);
