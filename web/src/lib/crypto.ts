 ...
 * Returns base64-encoded `iv` and `encrypted_value` (ciphertext + auth tag appended).
 */
export function encrypt(plaintext: string): { iv: string; encrypted_value: string } {
  const key = getKey();
  const iv = randomBytes(12); // 96-bit nonce for GCM
  const cipher = createCipheriv(ALGORITHM, key, iv);
  const ciphertext = Buffer.concat([cipher.update(plaintext, 'utf8'), cipher.final()]);
  const tag = cipher.getAuthTag(); // 16 bytes
  // Store ciphertext + tag concatenated so we only need two DB columns
