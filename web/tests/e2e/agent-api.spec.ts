...
test.describe('Agent API Endpoints', () => {
  test('should accept bearer token authentication on POST /api/agent/sync', async ({ request }) => {
    ...
    // Should either succeed or reject with auth error (not 404 or 405)
    expect([200, 400, 401, 403].includes(response.status())).toBe(true);
  });

  test('POST /api/agent/sync should return valid response', async ({ request }) => {
    const response = await request.post(`${BASE_URL}/api/agent/sync`, {
      headers: {
        'Authorization': `Bearer ${AGENT_API_KEY}`,
        'Content-Type': 'application/json',
  ...
  test('POST /api/agent/generate should be callable', async ({ request }) => {
  ...
  test('GET /api/agent/queue should retrieve queue status', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/api/agent/queue`, {
      headers: {
        'Authorization': `Bearer ${AGENT_API_KEY}`,
      },
    });
